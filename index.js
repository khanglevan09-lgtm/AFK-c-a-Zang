...
const OPTIONS = {
  ...
  port: 19000,               // Đã chỉnh sang Port 19000 chuẩn theo log của bạn
  username: 'Kiru',   
  hideErrors: true,
  checkTimeoutInterval: 90 * 1000 // FIX 1: Tăng chờ timeout lên 90s (Tránh lỗi client timed out)
};
...
// --- HÀM DỌN DẸP SẠCH BỘ NHỚ & TIMERS ---
function cleanupBot() {
...
function createBot() {
  ...
  bot.on('spawn', () => {
    ...
    if (isFirstSpawn) {
      ...
      // FIX 2: Gửi packet duy trì socket nhẹ nhàng mỗi 15 giây (Giúp không bị ECONNRESET)
      if (keepAliveInterval) clearInterval(keepAliveInterval);
      keepAliveInterval = setInterval(() => {
        if (bot && bot.entity && bot._client && !bot._client.socket.destroyed) {
          try {
            bot.look(bot.entity.yaw + 0.02, bot.entity.pitch, true);
          } catch (e) {}
        }
      }, 15000);
      ...
      loginTimer2 = setTimeout(() => {
        if (bot && bot._client && !bot._client.socket.destroyed) {
          ...
          // FIX 3: CHỈ KÍCH HOẠT CLICK CHUỘT SAU KHI ĐÃ VÀO SERVER HOÀN TOÀN
          startAutoClicker();
        }
      }, 20000);

      // 3. GIÁM SÁT KẾT NỐI (HEALTH CHECK)
      ...
      healthCheckInterval = setInterval(() => {
        ...
        if (isSocketDead) {
          console.log('[CẢNH BÁO] ⚠️ Phát hiện mất kết nối ngầm (Ghost Connection). Kết nối lại!');
          handleReconnect(); 
        } else {
          console.log('[GIÁM SÁT] ✅ Tình trạng kết nối ổn định.');
        }
      }, 600 * 1000);
    }
  });
...
// Bật clicker sau khi hoàn tất đăng nhập
function startAutoClicker() {
  ...
  }, 500); // Đã chỉnh lên 500ms để hoạt động mượt mà hơn
}
...
function handleReconnect() {
  ...
  reconnectTimeout = setTimeout(() => {
    failCount++;
    console.log(`[RECONNECT] 🔄 Đang thử kết nối lại... (Lần thứ ${failCount})`);
    createBot();
  }, waitTime * 1000);
}
