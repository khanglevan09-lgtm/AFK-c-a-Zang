...
function activateKeyForIp(userIp, inputKey) {
  ...
  // Cho phép chấp nhận các Key bắt đầu bằng "KEY_" như mặc định nếu muốn mở rộng
  if (!keyObj) {
    ...
    return { success: false, message: `Key [${inputKey}] không hợp lệ hoặc không tồn tại trên hệ thống!` };
  }
  ...
  return {
    success: true,
    message: `Kích hoạt thành công gói [${foundCategory}]! Key còn dùng được cho ${remaining} IP khác.`
  };
}
...
// ENDPOINT KÍCH HOẠT KEY TỪ WEB (TRẢ VỀ JSON XỬ LÝ NGẦM)
...
// ENDPOINT GỬI LỆNH CHAT NGẦM (JSON)
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* KHUNG NHẬT KÝ THÔNG BÁO TỔNG THỂ TRỰC TIẾP */
    .log-console {
      background: #11111b;
  ...
  <script>
    // 1. Hàm thêm Nhật ký trực tiếp vào khung hiển thị
    function addLog(message, isSuccess = true) {
      ...
      const color = isSuccess ? '#a6e3a1' : '#f38ba8'; // Xanh lá nếu thành công, Đỏ nếu lỗi
      const icon = isSuccess ? '✅' : '❌';

      logItem.innerHTML = `<span style="color: #6c7086;">[\${timeStr}]</span> \${icon} <span style="color: \${color};">\${message}</span>`;
      
      logBox.appendChild(logItem);
      logBox.scrollTop = logBox.scrollHeight; // Auto-scroll xuống dòng mới nhất
    }

    // 2. Kích hoạt Key trực tiếp không reload trang
    async function activateKeyDirect(keyOverride) {
      ...
      try {
        ...
        if (data.success) {
          ...
          // Lưu key vào localStorage để tự động khôi phục nếu Server bị reset RAM
          localStorage.setItem('saved_bot_key', keyVal);
          if (keyInput) keyInput.value = '';

          // Reload nhẹ sau 1 giây để mở các nút đang bị khóa
    ...
    // 3. Tự động khôi phục Key kích hoạt từ bộ nhớ Trình duyệt nếu Server vừa reset
    ...
    // 6. Gửi lệnh Chat trực tiếp từ Web
    ...
    // Tự động Reload kiểm tra (Chỉ reload khi người dùng không gõ chữ)
    setInterval(() => { 
      const activeEl = document.activeElement;
      if (!activeEl || activeEl.tagName !== 'INPUT') { location.reload(); }
    }, 8000);

    // Đổi Background Anime
...
<body>
  ...
  <div class="container">
    <div>
      <!-- KHU VỰC NHẬP KEY & NHẬT KÝ THÔNG BÁO -->
